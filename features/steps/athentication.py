...
def step_impl(context, username, password):
    ...
    centro.save()
    user = WebUser.objects.create(is_trabajador = True, password=password, username=username)
    user.save()
    trabajador = Trabajador.objects.create(User=user, nombre="Pepe",apellidos="Marin",telefono=698524587,email="email@email.email",anos=93, departamento="departamento", adresa="adresa",
            centro=centro)
    trabajador.save()
...
def step_impl(context, username, password):
    ...
    print("username:",trab.User.password,"\n")
    print("pasus:",username,"\n")
    print("pascon:",password,"\n")
